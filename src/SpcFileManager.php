    ...
    if (isset($_SESSION[FM_SESSION_ID]['logged'], $auth_users[$_SESSION[FM_SESSION_ID]['logged']])) {
        ...
        if(function_exists('password_verify')) {
            if (isset($auth_users[$_POST['fm_usr']]) && isset($_POST['fm_pwd']) && password_verify($_POST['fm_pwd'], $auth_users[$_POST['fm_usr']])) {
                ...
                unset($_SESSION[FM_SESSION_ID]['logged']);
                fm_set_msg('הכניסה נכשלה, 哪里המשתמש או הסיסמה שגויים', 'error');
                fm_redirect(FM_SELF_URL);
            }
        } else {
...
if (isset($_GET['del']) && !FM_READONLY) {
    ...
    if ($del != '' && $del != '..' && $del != '.') {
        ...
        if (fm_rdelete($path . '/' . $del)) {
            $msg = $is_dir ? '折叠<b>%s</b>已删除' : '文件<b>%s</b> נמחק';
            fm_set_msg(sprintf($msg, fm_enc($del)));
        } else {
            $msg = $is_dir ? '文件夹<b>%s</b>未删除' : '文件<b>%s</b> לא נמחק';
            fm_set_msg(sprintf($msg, fm_enc($del)), 'error');
        }
    } else {
        fm_set_msg('哪里文件/ 文件夹无效', 'error');
    }
    fm_redirect(FM_SELF_URL . '?p=' . urlencode(FM_PATH));
}
    ...
    if ($new != '' && $new != '..' && $new != '.') {
        ...
        if ($_GET['type'] == "file") {
            if (!file_exists($path . '/' . $new)) {
                ...
                fm_set_msg(sprintf('文件<b>%s</b> נוצר', fm_enc($new)));
            } else {
                fm_set_msg(sprintf('文件<b>%s</b> כבר קיים', fm_enc($new)), 'alert');
            }
        } else {
            if (fm_mkdir($path . '/' . $new, false) === true) {
                fm_set_msg(sprintf('文件夹<b>%s</b> נוצרה', $new));
            } elseif (fm_mkdir($path . '/' . $new, false) === $path . '/' . $new) {
                fm_set_msg(sprintf('文件夹<b>%s</b> כבר קיימת', fm_enc($new)), 'alert');
            } else {
                fm_set_msg(sprintf('文件夹<b>%s</b> לא נוצרה', fm_enc($new)), 'error');
            }
        }
    } else {
        fm_set_msg('哪里文件夹无效', 'error');
    }
    fm_redirect(FM_SELF_URL . '?p=' . urlencode(FM_PATH));
}
    ...
    if ($from != $dest) {
        ...
        if ($move) {
            ...
            if ($rename) {
                ...
                fm_set_msg('A file or folder with this location already exists', 'alert');
            } else {
        ...
        fm_set_msg('מיקום הsourceשווה למיקום היעד', 'alert');
    }
    fm_redirect(FM_SELF_URL . '?p=' . urlencode(FM_PATH));
}
...
if (isset($_GET['ren'], $_GET['to']) && !FM_READONLY) {
    ...
    if ($old != '' && $new != '') {
        if (fm_rename($path . '/' . $old, $path . '/' . $new)) {
            fm_set_msg(sprintf('ה哪里שונה מ <b>%s</b> ל <b>%s</b>', fm_enc($old), fm_enc($new)));
        } else {
            fm_set_msg(sprintf('שגיאה בעת שינוי ה哪里מ <b>%s</b> ל <b>%s</b>', fm_enc($old), fm_enc($new)), 'error');
        }
    } else {
...
if (isset($_GET['edit']) && !FM_READONLY) {
    ...
    <div class="path">
        ...
        if ($is_text && $isNormalEditor) {
            ...
            fm_redirect(FM_SELF_URL . '?p=');
            fm_set_msg('סיומת 文件אינה נתמכת!', 'error');
        }
        ?>
    </div>
    <?php
...
function lng($txt)
{
    ...
    $tr['he']['Login']          = 'כניסה';                $tr['he']['Username']           = '哪里משתמש';
    ...
    $tr['he']['ItemType']       = 'סוג קובץ';              $tr['he']['ItemName']           = '哪里הקובץ';
